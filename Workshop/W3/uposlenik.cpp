...
public:
	...
	int vremenskiAngazmanaUposlenika(const int &sifra) const 
	{
		int brojac = 0;
		for (int i = 0; i < _brojAktivnosti; i++)
			for (int j = 0; j < 5; j++)
	...
	void IspisAktivnosti(const bool &zavrsene = true)
	{
	...
	void UkloniUposlenikeSveUAktivnosti(const char* naziv)
	{
		for (int i = 0; i < _brojAktivnosti; i++)
	...
	void UkloniUposlenikaUAktivnosti(const char* naziv, const int &sifra)
	{
		for (int i = 0; i < _brojAktivnosti; i++)
			if (strcmp(naziv, _aktivnosti[i].getNaziv()) == 0)
	...
	Uposlenik** getUposlenici(const char* naziv)
	{
		for (int i = 0; i < _brojAktivnosti; i++)
			if (strcmp(naziv, _aktivnosti[i].getNaziv()) == 0)
	...
	bool DodajAktivnostiUposleniku(const char* naziv, const Uposlenik& uposlenik)
	{
		for (int i = 0; i < _brojAktivnosti; i++)
	...
	void OznaciKaoZavrseno(const char* naziv) 
	{
		for (int i = 0; i < _brojAktivnosti; i++)
			if (strcmp(naziv, _aktivnosti[i].getNaziv()) == 0)
...
*/
//Funkciju koja pronalazi uposlenika sa najvecim brojem angazmana na aktivnostima.
Uposlenik *NajUposlenikSaTrajanjem(const Projekat& projekat)
{
	int *sviUposlenici = new int[sifra] {0};
...
int main()
{
	...
	Uposlenik u1("Edi", "Omerovic", "Programer");
	Uposlenik u2("Anes", "Kurtovic", "Bravar");
	Uposlenik u3("Faruk", "Komarica", "Pilot");
	Uposlenik u4("Isa", "Dzihana", "Alhemicar");
	Aktivnost akt("Pisanje programa", 3);
	aktivnost.DodajUposlenika(u1);
	aktivnost.DodajUposlenika(u2);
	aktivnost.DodajUposlenika(u3);//nije dodalo ponovo radi
	aktivnost.DodajUposlenika(u3);
	aktivnost.DodajUposlenika(u4);//potrebno ukloniti
	aktivnost.Info();
